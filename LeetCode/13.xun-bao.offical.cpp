...
class Solution {
 public:
  ...
  vector<vector<int>> bfs(int x, int y, vector<string>& maze,vector<vector<int>> &ret)const {
    ret[x][y] = 0;
  ...
  int minimalSteps(vector<string>& maze) {
    ...
    int nb = buttons.size();
    int ns = stones.size();
    vector<vector<int>> start_dist(n, vector<int>(m, -1));
    bfs(sx, sy, maze,start_dist);
    ...
    for (int i = 0; i < nb; i++) {
      ...
      bfs(buttons[i].first, buttons[i].second, maze, d);
      dd[i] = d;
      // 从某个点到终点不需要拿石头
      dist[i][nb + 1] = d[tx][ty];
    }
